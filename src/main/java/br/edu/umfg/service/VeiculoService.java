    ...
    public Veiculo cadastrar(Veiculo veiculo) throws SQLException {
        ...
        if (clienteRepository.buscarPorId(veiculo.getIdCliente()) == null) {
            throw new IllegalArgumentException(
                    "[REQUISITO] Titular de ID " + veiculo.getIdCliente() + " não localizado. Realize a abertura do cadastro do cliente primeiro."
            );
        }
        if (veiculo.getPlaca() == null || veiculo.getPlaca().trim().isEmpty()) {
            throw new IllegalArgumentException("[REQUISITO] A identificação da placa automotiva não pode ficar em branco.");
        }
        if (veiculo.getAno() < 1900 || veiculo.getAno() > 2100) {
            throw new IllegalArgumentException("[REQUISITO] O ano de fabricação informado está fora dos parâmetros aceitáveis.");
        }
