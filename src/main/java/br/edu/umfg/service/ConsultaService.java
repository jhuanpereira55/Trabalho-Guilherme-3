...
public class ConsultaService {
    ...
    public void cadastrar(Consulta consulta) {
        if (animalRepository.buscarPorId(consulta.getIdAnimal()) == null) {
            throw new RuntimeException("[VALIDAÇÃO] O paciente animal informado não foi localizado no sistema.");
        }
        if (consulta.getValor() < 0) {
            throw new RuntimeException("[VALIDAÇÃO] O valor financeiro estipulado para a consulta não pode ser negativo.");
        }
        if (consulta.getMotivo() == null || consulta.getMotivo().isBlank()) {
            throw new RuntimeException("[VALIDAÇÃO] A descrição do motivo do atendimento médico é obrigatória.");
        }
    ...
    public Consulta buscarPorId(int id) {
        ...
        if (consulta == null) {
            throw new RuntimeException("[VALIDAÇÃO] Nenhuma consulta foi encontrada com o ID informado.");
        }
        return consulta;
    }
